...
const KanbanBoard: React.FC<KanbanBoardProps> = ({
  ...
  // Função otimizada para mover tarefa - APENAS 1 REQUISIÇÃO
  const handleDragEnd = useCallback(async (result: DropResult) => {
    ...
    // 2. SINCRONIZAÇÃO SILENCIOSA COM BACKEND (sem loading visual)
    try {
      // BACKGROUND: apenas uma chamada direta sem loading visual
      ...
      // Mostrar erro para o usuário de forma não intrusiva
      showError('Erro ao mover tarefa. A operação foi desfeita.');
    } finally {
      previousStateRef.current = null;
    }
  }, [onTaskMove, showError]);

  // Callback para início do drag (apenas para visual feedback)
  ...
  const renderColumn = useCallback((column: ColumnType) => {
    const columnTasks = column.taskIds
      ...
      .filter(Boolean)
      .sort((a, b) => {
        // Ordenar por posição se disponível, senão por data de criação
  ...
  }, [tasks, columns.length, onColumnAdd, onColumnDelete, renderTask]);  return (
    <>
      {error && (
